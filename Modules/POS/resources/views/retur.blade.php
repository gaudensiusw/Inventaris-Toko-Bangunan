...
<div class="space-y-6">
    ...
    @if($selected_trx)
        ...
        <div class="bg-white rounded-2xl border border-rose-200 shadow-md overflow-hidden" x-data="{
            ...
            alasan: 'Salah Beli / Ganti Ukuran',
            toggleItem(idx) {
            ...
            <!-- Form -->
            <form action="{{ route('pos.retur.process') }}" method="POST" class="p-6 space-y-6">
                @csrf
                    ...
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="bg-slate-100 text-slate-600 uppercase font-black border-b border-slate-200">
                                ...
                                <th class="py-3 px-4 w-32">Qty Retur</th>
                                <th class="py-3 px-4 w-48">Kondisi Barang</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <template x-for="(item, idx) in items" :key="idx">
                                <tr :class="item.selected ? 'bg-rose-50/40' : 'hover:bg-slate-50/60'">
                                    ...
                                    <td class="py-3 px-4">
                                        <select :name="`items[${idx}][kondisi]`" x-model="item.kondisi" :disabled="!item.selected" 
                                                class="w-full border border-slate-300 rounded-lg text-xs p-2 font-bold text-slate-700 bg-white disabled:bg-slate-100">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-slate-200 gap-4">
                    <div>
                        @if($selected_trx->metode_pembayaran === 'Bon')
                            <p class="text-xs text-amber-600 font-bold bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200">
                                ℹ️ Transaksi Bon: Total refund akan **memotong sisa hutang pelanggan** pada transaksi ini.
                            </p>
                        @else
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <button type="submit" class="px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white font-bold text-sm rounded-xl shadow-lg shadow-rose-100 transition-all flex items-center gap-2">
