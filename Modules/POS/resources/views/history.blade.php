                    ...
                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" @click="expandedTrx = (expandedTrx === {{ $t->id }} ? null : {{ $t->id }})">
                        ...
                        <td class="p-4 text-center whitespace-nowrap" @click.stop>
                            <div class="flex items-center justify-center gap-1.5">
                                ...
                                @php
                                    $hasRefunds  = $t->refunds->count() > 0;
                                    $detailCount = $t->details->count();
                                    $returStatus = !$hasRefunds ? 'none'
                                        : ($detailCount > 0 ? 'partial' : 'full');
                                @endphp
                                @if($returStatus === 'full')
                                    <span class="px-2.5 py-1 bg-slate-100 text-slate-500 font-bold text-xs rounded border border-slate-200 flex items-center gap-1 cursor-not-allowed" title="Sudah diretur penuh">
                                ...
                                @elseif($returStatus === 'partial')
                                    <a href="{{ route('pos.retur.index', ['search' => $t->no_transaksi]) }}" class="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold text-xs rounded border border-amber-300 flex items-center gap-1 transition-all" title="Ada retur sebagian — klik untuk retur sisa">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z"></path></svg>
                                        Retur Parsial
                                    </a>
                                @else
